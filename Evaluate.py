...
from Model import TimeSeriesCNNLSTM, parse_args
...
def predict_future(model, initial_sequence, n_steps, device):
    ...
    current_sequence = initial_sequence.clone().detach().to(device)
    
    with torch.no_grad():
        for _ in range(n_steps):
            current_sequence = current_sequence.unsqueeze(-1)
            
            prediction = model(current_sequence.unsqueeze(0).float())
            
            predictions.append(prediction.item())
            
            current_sequence = torch.cat((current_sequence.squeeze(-1)[1:], prediction.squeeze(-1)))
